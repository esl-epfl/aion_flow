...
def _run_cmd_status(
    ...
    """Run ``cmd`` and return ``(exit_status, combined output)``.

    The status is the value the DRC receipt records, and it is the only thing
    that distinguishes "the tool ran and found violations" (1) from "the tool
    never finished" (anything else).  A boolean cannot carry that.
    """
    proc = subprocess.run(
        list(cmd),
        cwd=cwd,
...
def _run_cmd(cmd: Sequence[str], cwd: Optional[Path] = None) -> _RunResult:
    status, output = _run_cmd_status(cmd, cwd)
    return status == 0, output


def write_klayout_receipt(directory: os.PathLike[str], exit_status: int,
                          *, tool: str = "klayout", cell: str = "") -> Optional[Path]:
    """Record what the KLayout half of a DRC run produced, beside the databases.

    Written by the runner *before* anything parses the result, because the merge
    refuses to call a zero-item result clean without it -- see the receipt
    section at the top of this module.  Returns the receipt path, or ``None``
    when ``directory`` does not exist (KLayout never ran), which the merge then
    grades as UNVERIFIED rather than clean.
    """
    ...
    payload = {
        ...
        "databases": sorted(p.name for p in directory.glob("*.lyrdb") if p.is_file()),
    }
    path = directory / KLAYOUT_RECEIPT_NAME
    tmp = directory / f".{KLAYOUT_RECEIPT_NAME}.tmp"
...
def run_drc(
    ...
    exit_status, output = _run_cmd_status(
        [
            str(script),
            "sak-drc.sh",
            "-d", "-b", "-l", "macro",
    ...
    # The receipt has to exist before the merge reads the databases: without it
    # a deleted rule table is invisible and a zero-item result reads clean.
    write_klayout_receipt(
        work_dir / f"{cell_name}.{KLAYOUT_DRC_DIR_SUFFIX}",
        exit_status,
        cell=cell_name,
    )
