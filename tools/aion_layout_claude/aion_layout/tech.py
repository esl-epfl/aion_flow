...
def _build_sg13g2_design_rules() -> dict[str, Any]:
    ...
    return {
        ...
        "min_spacing_nm": {
            ...
            "Metal1": 180.0,
            "Metal2": 210.0,
            "Via1": 220.0,  # V1.b; V1.b1 raises this to 290 in one direction
            # for arrays of more than 3 rows *and* more than 3 columns.
            "NWell": 620.0,
        },
        # Pair-wise spacing rules (layer_a, layer_b) in nanometres.
        "min_spacing_nm_pairs": {
            # Cnt.f: min. Cont-on-Activ space to GatPoly.  This was 70 nm, which
            # is Gat.d (GatPoly space to Activ) applied to the wrong pair; every
            # source/drain contact placed with it lands 40 nm inside the Cnt.f
            # keep-out.  The shipped sg13g2 cells use the 110 nm pitch this
            # implies (110 + 160 + 110 + 130 = 510 nm contacted poly pitch).
            ("GatPoly", "Cont"): 110.0,
            ("GatPoly", "Activ"): 70.0,
        },
        "min_enclosure_nm": {
            ...
            #
            # These are the values you must *draw*, not the smallest number a
            # rule mentions.  Every cut has two enclosure tiers: an all-sides
            # rule and a larger "endcap" rule that is waived on one side, or on
            # two opposite sides.  Sizing a landing from the all-sides number
            # alone produces an endcap violation on the two remaining sides, so
            # the endcap value is what belongs here.
            #
            #   Via1:  V1.c  = 10 nm Metal1 all sides, V1.c1 = 50 nm endcap
            #          M2.c  =  5 nm Metal2 all sides, M2.c1 = 50 nm endcap
            #   Cont:  M1.c  =  0 nm Metal1 all sides, M1.c1 = 50 nm endcap
            #          (70 nm is kept: a Cont landing grown by 50 nm is
            #          260 x 260 nm = 0.0676 um2 and fails M1.d, 0.09 um2;
            #          grown by 70 nm it is 300 x 300 nm = 0.09 um2 exactly.)
            #
            # ``building_blocks`` carries the all-sides numbers for Via1 so a
            # minimum-width Metal2 wire can still legally carry a via.
            "Metal1": {"Cont": 70.0, "Via1": 50.0},
            "Metal2": {"Via1": 50.0},
            "NWell": {"PSD": 310.0},
            "PWell": {"NSD": 310.0},
            "NSD": {"Cont": 90.0},
        ...
        "via_size_nm": {
            # Both are min *and* max rules (Cnt.a, V1.a): a cut is one size,
            # never "at least" a size.
            "Cont": 160.0,
            "Via1": 190.0,
        },
        # Minimum polygon area, in square nanometres.  A shape can pass every
        # width and spacing rule and still be illegal because it is too small;
        # Metal2's 0.144 um2 is what forces a 200 nm wide Metal2 rectangle to
        # be at least 720 nm long.
        "min_area_nm2": {
            "Activ": 122000.0,   # Act.d  0.122 um2
            "GatPoly": 90000.0,  # Gat.e  0.09  um2
            "Metal1": 90000.0,   # M1.d   0.09  um2
            "Metal2": 144000.0,  # M2.d   0.144 um2
        },
