...
class Cell:
    ...
    def _insert_boundary(self, layout: pya.Layout, top: pya.Cell) -> None:
        """Draw the prBoundary rectangle ``set_boundary`` was given.

        ``set_boundary`` used to record the rectangle for :attr:`bbox` and
        nothing else, so a generator could set a perfectly good boundary and
        still write a GDS with no prBoundary layer in it.  Everything
        downstream then measured the *shape bounding box* instead -- which
        includes the well and implant overhang a neighbour would share, and so
        reports a cell as larger the better it abuts.  The placement footprint
        has to be in the artifact, not only in the object that wrote it.

        A generator that already draws its own prBoundary is left alone: the
        rectangle is inserted only when the layer carries nothing yet.
        """
        if self._boundary is None:
            return
        layer = self.tech.get("prBoundary") if self.tech else None
        if layer is None:
            return
        layer_index = layout.layer(layer.gds_layer, layer.gds_datatype)
        if not top.shapes(layer_index).is_empty():
            return
        top.shapes(layer_index).insert(
            pya.Box(
                int(round(self._boundary.left)),
                int(round(self._boundary.bottom)),
                int(round(self._boundary.right)),
                int(round(self._boundary.top)),
            )
        )
    ...
    def write_gds(self, path: str | Path) -> None:
        ...
        self._insert_boundary(layout, top)
