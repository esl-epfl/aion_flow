...
`aion_opt` takes a post-synthesis netlist, mines recurring combinational patterns, generates new structural Verilog cells for those patterns, and rewrites the netlist to use them. The result is a hierarchical optimized netlist, a flat equivalent, the generated AION cell library, and reports.

All outputs land under `build/aion_opt/` by default when run through the root `Makefile`.
...
Run logical equivalence check (LEC) and sequential equivalence check (SEC) on the produced netlists. These invoke `scripts/verify/run_lec_sec.py`, which in turn runs `kepler-formal` inside the EDA container via `scripts/docker_run.sh`.
